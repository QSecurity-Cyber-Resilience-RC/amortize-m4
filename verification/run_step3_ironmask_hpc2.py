...
def require_expected_gadgets_c(gadgets_c: Path) -> None:
    ...
    required_patterns = [
        r"void\s+isw_and\s*\(",
        r"void\s+hpc2_and\s*\(",
        r"r\s*\[\s*i\s*\]\s*\[\s*j\s*\]\s*=\s*rr",
        r"r\s*\[\s*j\s*\]\s*\[\s*i\s*\]\s*=\s*rr",
        r"uint32_t\s+t1\s*=\s*opt_bar\s*\(\s*\(\s*uint32_t\s*\)\s*\(\s*~a\s*\[\s*i\s*\]\s*\)\s*&\s*rij\s*\)",
        r"uint32_t\s+bm\s*=\s*opt_bar\s*\(\s*b\s*\[\s*j\s*\]\s*\^\s*rij\s*\)",
        r"uint32_t\s+t2\s*=\s*opt_bar\s*\(\s*a\s*\[\s*i\s*\]\s*&\s*bm\s*\)",
...
def gen_hpc2_and_exact(shares: int) -> str:
    """
    Generate IronMask syntax for gadgets.c:hpc2_and.

    Important modeling note:
        The literal C schedule contains random-dependent multiplications:

            t1 = (~a[i]) & rij
            bm = b[j] ^ rij
            t2 = a[i] & bm

        A direct IronMask encoding of this schedule fails for order >= 2 with
        messages such as:

            Unsupported format for random ... in a multiplication gadget
            Unsupported format for variable ... in a multiplication gadget

        because IronMask's multiplication-gadget parser expects AND operands
        to be base input shares, not random-dependent intermediate expressions.

        Therefore this generator emits the algebraically reduced Boolean form
        of the same output function:

            (~a_i & r_ij) ^ (a_i & (b_j ^ r_ij))
          = (r_ij ^ (a_i & r_ij)) ^ (a_i & b_j) ^ (a_i & r_ij)
          = r_ij ^ (a_i & b_j)

        Hence each inner-loop contribution is encoded as:

            cross = a[i] * b[j]
            acc   = acc + rij + cross

        This avoids randoms inside multiplication operands and is accepted by
        IronMask. It is functionally equivalent at the Boolean bit level, but
        it is NOT the exact C intermediate schedule. Use the generated
        transcripts as evidence for the reduced hpc2 Boolean function. For a
        strict probing proof of the exact C schedule, use a verifier/encoding
        that supports random-dependent AND operands.
    """
    order = shares - 1
    randoms = [f"r{i}{j}" for i in range(shares) for j in range(i + 1, shares)]
    lines: List[str] = []

    def rname(i: int, j: int) -> str:
        lo = min(i, j)
        hi = max(i, j)
        return f"r{lo}{hi}"

    lines.append("# Generated from gadgets.c:hpc2_and algebraically reduced Boolean model\n")
    lines.append("# Boolean field convention: + = XOR, * = AND.\n")
    lines.append("# Reduction used: ((~a_i)&r_ij) + (a_i*(b_j+r_ij)) = r_ij + (a_i*b_j).\n")
    lines.append("# This avoids random-dependent multiplication operands unsupported by IronMask.\n")
    lines.append("# Symmetric randoms: r[i][j] = r[j][i] = r_min_max.\n\n")

    for i in range(shares):
        lines.append(f"# Output share c{i}\n")
        lines.append(f"# acc = a[{i}] & b[{i}]\n")
        lines.append(f"acc{i} = a{i} * b{i}\n")
        ...
        for j in range(shares):
            ...
            rij = rname(i, j)
            lines.append("\n")
            lines.append(f"# j = {j}, rij = r[{i}][{j}] = {rij}\n")
            lines.append(f"# reduced contribution = rij ^ (a[{i}] & b[{j}])\n")
            lines.append(f"cross_{i}_{j} = a{i} * b{j}\n")
            lines.append(f"acc{i} = acc{i} + {rij}\n")
            lines.append(f"acc{i} = acc{i} + cross_{i}_{j}\n")

        lines.append(f"\n# c[{i}] = acc\n")
        lines.append(f"c{i} = acc{i}\n\n")
...
def main() -> int:
    ...
    for order in range(1, args.max_order + 1):
        ...
        hpc2_path = inputs_dir / f"ours_hpc2_and_{shares}_shares.sage"
        ...
        write_text(hpc2_path, gen_hpc2_and_exact(shares))
        ...
        # hpc2_and: run NI/SNI by default. PINI optional.
        # This models the provided HPC2-style AND schedule with symmetric
        # pairwise randoms and bit-level NOT represented as 1 + a_i.
        for prop in ["NI", "SNI"]:
            jobs.append(Job(
                gadget="hpc2_and",
                prop=prop,
                order=order,
                sage_file=hpc2_path,
                transcript_file=transcripts_dir / f"ours_hpc2_and_{prop.lower()}_order{order}.txt",
                timeout_seconds=args.timeout,
            ))
        ...
        if args.include_pini:
            jobs.append(Job(
                ...
                order=order,
                sage_file=hpc2_path,
                transcript_file=transcripts_dir / f"ours_hpc2_and_pini_order{order}.txt",
                timeout_seconds=args.timeout,
            ))
