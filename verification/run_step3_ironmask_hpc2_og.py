...
def strip_non_directive_sage_comments(text: str) -> str:
    """
    Remove ordinary comment lines from generated .sage files while preserving
    IronMask metadata directives.

    IronMask directives intentionally start with '#':
        #ORDER, #SHARES, #IN, #RANDOMS, #OUT

    Those lines are required and must not be removed. This function removes
    every other line whose first non-whitespace character is '#'.
    """
    directive_prefixes = ("#ORDER", "#SHARES", "#IN", "#RANDOMS", "#OUT")
    cleaned_lines: List[str] = []

    for line in text.splitlines(keepends=True):
        stripped = line.lstrip()
        if stripped.startswith("#") and not stripped.startswith(directive_prefixes):
            continue
...
def write_sage_text(path: Path, text: str) -> None:
    """
    Write a generated IronMask .sage file without ordinary comments.

    Required IronMask directives are preserved.
    """
    write_text(path, strip_non_directive_sage_comments(text))
...
def gen_hpc2_and_from_reference_sage(shares: int) -> str:
    """
    Generate IronMask syntax for the HPC2 AND gadget using the attached
    hpc2_2.sage schedule as the reference model.

    For 2 shares, the generated file follows the reference shape:
    ...
    For n shares, the same per-output-share pattern is generalized:
        c_i = a_i*b_i + sum_{j != i} [ ((a_i + 1) * r_ij)
                                      + (a_i * (b_j + r_ij)) ]

    Note:
        This keeps the '+ 1' complement representation used by hpc2_2.sage.
        If a particular IronMask build rejects constants, use the constant-free
        generator variant instead.
    """
    if shares < 2:
    ...
    def rr(i: int, j: int) -> str:
        lo, hi = (i, j) if i < j else (j, i)
        return f"r{lo}{hi}"

    # Special naming for the 2-share file to match the attached hpc2_2.sage
    ...
    for i in range(shares):
        ...
        for j in range(shares):
            ...
            rij = rr(i, j)
            lines.append(f"w1_{i}_{j} = na{i} * {rij}\n")
            lines.append(f"m_{i}_{j} = b{j} + {rij}\n")
            lines.append(f"w2_{i}_{j} = a{i} * m_{i}_{j}\n")
            lines.append(f"cross_{i}_{j} = w1_{i}_{j} + w2_{i}_{j}\n")
            cross_names.append(f"cross_{i}_{j}")

        lines.append(f"p{i} = a{i} * b{i}\n")
        lines.append(f"c{i} = p{i} + {cross_names[0]}\n")
        for name in cross_names[1:]:
            lines.append(f"c{i} = c{i} + {name}\n")
        lines.append("\n")
...
def main() -> int:
    ...
    ap.add_argument("--include-pini", action="store_true",
                    help="Also run PINI for all machine-checkable generated gadgets. Some PINI checks may fail for a given encoding.")
    ...
    for order in range(1, args.max_order + 1):
        ...
        write_sage_text(isw_path, gen_isw_and_exact(shares))
        write_sage_text(hpc2_path, gen_hpc2_and_from_reference_sage(shares))
        write_sage_text(refresh_path, gen_refresh_sni_exact(shares))
        write_sage_text(linear_path, gen_linear_refresh_exact(shares))
        ...
        # hpc2_and: always generated and verified.
        # Run NI/SNI by default; run PINI too when --include-pini is set.
        for prop in ["NI", "SNI"]:
            jobs.append(Job(
                gadget="hpc2_and",
                prop=prop,
