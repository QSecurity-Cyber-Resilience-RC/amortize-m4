...
def gen_hpc2_and_from_reference_sage(shares: int) -> str:
    """
    Generate IronMask syntax for the HPC2 AND gadget.

    Important IronMask compatibility note:
        The attached reference hpc2_2.sage used constants such as:
            na0 = a0 + 1
        to model NOT(a0). This IronMask build does not accept literal
        constants in arithmetic expressions and fails with:
            Elem '' not found in map 'Dependencies'.

    Therefore this generator emits an algebraically equivalent Boolean-field
    form without constants.

    C/reference expression per cross term:
        cross_ij = ((~a_i) & r_ij) ^ (a_i & (b_j ^ r_ij))

    Boolean-field simplification, with + = XOR and * = AND:
        (~a_i) & r_ij = (1 + a_i) * r_ij
                         = r_ij + (a_i * r_ij)
        a_i & (b_j ^ r_ij) = a_i * (b_j + r_ij)
                            = (a_i * b_j) + (a_i * r_ij)
        cross_ij = r_ij + (a_i * r_ij) + (a_i * b_j) + (a_i * r_ij)
                 = r_ij + (a_i * b_j)

    The emitted circuit therefore models:
        acc_i = a_i*b_i
        for j != i:
            cross_ij = r_ij + a_i*b_j
            acc_i = acc_i + cross_ij
        c_i = acc_i

    This avoids constants and is accepted by IronMask. It is algebraically
    equivalent to the provided reference HPC2 schedule at the Boolean bit level,
    but it does not preserve the exact intermediate variables involving NOT.
    For compiled-code leakage/transition analysis, use Step 4/TVLA.
    """
    if shares < 2:
    ...
    lines.append("# Generated HPC2 AND model, constant-free IronMask encoding\n")
    lines.append("# Derived from the provided hpc2_2.sage reference schedule\n")
    lines.append("# + = XOR, * = AND\n")
    lines.append("# cross_ij = ((~a_i)&r_ij) + (a_i*(b_j+r_ij))\n")
    lines.append("#          = r_ij + (a_i*b_j) over GF(2)\n\n")
    ...
    for i in range(shares):
        ...
        for j in range(shares):
            ...
            lines.append(f"tmp_a{i}_b{j} = a{i} * b{j}\n")
            lines.append(f"cross_{i}_{j} = {rij} + tmp_a{i}_b{j}\n")
...
def main() -> int:
    ...
    ap.add_argument("--include-hpc2", action="store_true",
                    help="Generate and verify hpc2_and using the schedule generalized from the provided hpc2_2.sage reference file.")
    ...
    metadata = (
        ...
        f"include_hpc2: {args.include_hpc2}\n\n"
        "Note:\n"
    ...
    for order in range(1, args.max_order + 1):
        ...
        if args.include_hpc2:
            write_text(hpc2_path, gen_hpc2_and_from_reference_sage(shares))
        ...
        # hpc2_and: optional alternative AND gadget generated from hpc2_2.sage.
        # Run NI/SNI by default when included; run PINI too when --include-pini is set.
        if args.include_hpc2:
            for prop in ["NI", "SNI"]:
                jobs.append(Job(
                    gadget="hpc2_and",
